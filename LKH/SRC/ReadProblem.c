 ...
 * TOR_2D       Weighrs are toroidal distances in 2-D
...
void ReadProblem()
{
    ...
    C = 0;
    c = 0;
    while ((Line = ReadStrLine(&ProblemInput))) {
        ...
        if (!strcmp(Keyword, "COMMENT"));
            ...
            eprintf("TOUR_SECTION in the problem file is not implemented at this time.");
...
static void Read_EDGE_DATA_SECTION()
{
    ...
    if (!strcmp(EdgeDataFormat, "EDGE_LIST")) {
        ...
        while (i != -1) {
            ...
            if (WithWeights) {
                if (!FirstLine)
                    scanint(&ProblemInput, &W);
                W *= Precision;
            }
            AddCandidate(Ni, Nj, W, 1);
            AddCandidate(Nj, Ni, W, 1);
            FirstLine = 0;
            if (!scanint(&ProblemInput, &i))
                i = -1;
        }
